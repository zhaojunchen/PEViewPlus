...
public:
    ...
    // DosStud  is start at dos_header+sizeof(IMAGE_DOS_HEADER) size is
    // (dos_header->e_lfanew-sizeof(IMAGE_DOS_HEADER))
    PIMAGE_NT_HEADERS64 nt_header; // (PIMAGE_NT_HEADERS32)((char*)dos_header+(dos_header->e_lfanew))
    PIMAGE_DATA_DIRECTORY data_directory;
    PIMAGE_SECTION_HEADER section_header;
    ...
    Node* init_nt_headers_optional_header(int startVA = 0) {
        ...
        auto it6 = header->AddressOfEntryPoint;
        auto it7 = header->BaseOfCode;
        auto it8 = header->ImageBase;
        auto it9 = header->SectionAlignment;
        auto it10 = header->FileAlignment;
        auto it11 = header->MajorOperatingSystemVersion;
        auto it12 = header->MinorOperatingSystemVersion;
        auto it13 = header->MajorImageVersion;
        auto it14 = header->MinorImageVersion;
        auto it15 = header->MajorSubsystemVersion;
        auto it16 = header->MinorSubsystemVersion;
        auto it17 = header->Win32VersionValue;
        auto it18 = header->SizeOfImage;
        auto it19 = header->SizeOfHeaders;
        auto it20 = header->CheckSum;
        auto it21 = header->Subsystem;
        auto it22 = header->DllCharacteristics;
    ...
    void init_section_header_1(int RVA, Node *node,
        ...
        // 设置Name
        node->addr.push_back(Addr(RVA, 8));
        RVA += 4;
        node->addr.push_back(Addr(RVA, 8));
        RVA += 4;
    ...
    QVector<Node *>init_rdata() {
        ...
        for (int i = 0; i < N_IID; i++) {
            ...
            for (int i = 0; i < 5; i++) { // all element is 4 bytes
                node_IDT->addr.push_back(Addr(IID_RVA, 8));
                node_IDT->desc.push_back(desc[i]);
            ...
            while ((value = INT->u1.Ordinal) != 0) {
                ...
                node_IAT->addr.push_back(Addr(RVA_IAT, 8));
                node_INT->addr.push_back(Addr(RVA_INT, 8));
    ...
    Node* init_reloc_table() {
        ...
        while (size > 0) {
            // 资源节块的四个起始内容
            IDR_RVA_BAKE = IDR_RVA + startVA;
            node->addr.push_back(Addr(IDR_RVA_BAKE, 8));
            IDR_RVA_BAKE += 4;
            node->addr.push_back(Addr(IDR_RVA_BAKE, 8));
            IDR_RVA_BAKE += 4;
    ...
    QVector<Node *>init_edata() {
        ...
        for (int i = 0; i < N; i++) {
            node_EDT->addr.push_back(Addr(RVA, 8));
            node_EDT->data.push_back(Addr(it_value[i], it_size[i]));
        ...
        for (int i = 0; i < N_Name; i++) {
            functoinName = (char *)(content + (*funName - dis));
            node_EOT->addr.push_back(Addr(EOT_RVA, 8));
            node_ENT->addr.push_back(Addr(ENT_RVA, 8));
            node_EOT->data.push_back(Addr(*funIndex, 2));
            node_ENT->data.push_back(Addr(*funName, 4));
            node_EOT->desc.push_back("Function ordinal");
            node_ENT->desc.push_back("Function Name PVA");
        ...
        for (int i = 0, j = 0; i < EDT->NumberOfFunctions; i++) {
            node_EAT->addr.push_back(Addr(EAT_RVA, 8));
            node_EAT->data.push_back(Addr(*addressOfFunctions, 4));
    ...
    void init_pe_allnodes() {
        ...
        // NT  HEADER
        nt_header =
            (PIMAGE_NT_HEADERS64)((char *)dos_header + (dos_header->e_lfanew));
    ...
    void fillContent(Node *node,
        ...
        for (int i = 0; i < turn; i++) {
            //  c 指向此段的内容
            node->addr.append(Addr(RVA_offset));
            RVA_offset += 16;
        }
        QString s;
