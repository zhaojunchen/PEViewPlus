...
int PrintReloactionTable_64(PVOID FileAddress)
{
	int ret = 0;
...
int PrintResourceTable_64(PVOID FileAddress)
{
	/*string szResType[0x11] = { 0, "鼠标指针", "位图", "图标", "菜单",
						 "对话框", "字符串列表", "字体目录", "字体",
						 "加速键", "非格式化资源", "消息列表", "鼠标指针组",
...
/*
	异常表
*/
int PrintExceptionTable(PVOID FileAddress) 
{
	int ret = 0;
	...
	//	(2)、获取表的FOA地址
	ret = RVA_TO_FOA(FileAddress, ExceptionDirectory_RAVAdd, &ExceptionDirectory_FOAAdd);
	if (ret != 0)
	{
		printf("func RVA_TO_FOA() Error!\n");
		return ret;
...
int PrintExceptionTable_64(PVOID FileAddress) 
{
	int ret = 0;
...
/*
	证书表(未完)
*/
int PrintReadSecurityTable(PVOID FileAddress) 
{
	int ret = 0;
	...
	//	(2)、获取证书表的FOA地址
	ret = RVA_TO_FOA(FileAddress, SecurityDirectory_RAVAdd, &SecurityDirectory_FOAAdd);
	if (ret != 0)
	{
		printf("func RVA_TO_FOA() Error!\n");
		return ret;
...
int PrintReadSecurityTable_64(PVOID FileAddress) 
{
	int ret = 0;
	return ret;
}
...
int main()
{
	...
	if (ret == 32) {
		//2、打印导入表信息
		ret_tmp = PrintImportTable(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//3、输出表
		ret_tmp = PrintExportTable(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//4、打印重定位表信息
		ret_tmp = PrintReloactionTable(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//5、打印资源表的信息
		ret_tmp = PrintResourceTable(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//6、打印异常表的信息
		ret_tmp = PrintExceptionTable(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		//64位程序
		//2、打印导入表信息64位
		ret_tmp = PrintImportTable_64(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//3、打印导出表信息64位
		ret_tmp = PrintExportTable_64(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//4、打印重定位表信息64位
		ret_tmp = PrintReloactionTable_64(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//5、打印资源表的信息
		ret_tmp = PrintResourceTable_64(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
		...
		printf("\n");
		//6、打印异常表的信息
		ret_tmp = PrintExceptionTable_64(FileAddress);
		if (ret_tmp != 0)
		{
			if (FileAddress != NULL)
				free(FileAddress);
